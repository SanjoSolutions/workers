...
3. If the task is already autonomous and safe for a worker to start immediately, add it straight to
   `## Ready to be picked up` with `node add-todo.js --ready`.
...
8. Add it with `node add-todo.js`. Workers will route it to the configured task tracker for the target
